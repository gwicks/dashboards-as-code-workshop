...
def deploy_dashboard(dash: dashboard.Dashboard):
    grafana = Grafana(GrafanaConfig.from_env())

    folder_uid = grafana.find_or_create_folder(DASHBOARD_FOLDER_NAME)
    grafana.persist_dashboard(folder_uid, dash)
    
    print("dashboards deployed")

def generate_manifests(dash: dashboard.Dashboard):
    grafana = Grafana(GrafanaConfig.from_env())
    dash = dash.build()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--deploy', action='store_true', help='Generate and deploy the test dashboard directly to a Grafana instance')
    parser.add_argument('--manifests', action='store_true', help='Generate a dashboard manifest for the test dashboard and write it to disk')
